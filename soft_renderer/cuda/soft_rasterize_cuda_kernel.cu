...
namespace
{
                                                           ...
                                                           const scalar_t xp, const scalar_t yp)
    {
        ...
        else
        {
            ...
            {
                v0 = 0;
                if (face_obt[0] == 1 && (xp - face[0]) * (face[6] - face[0]) + (yp - face[1]) * (face[7] - face[1]) > 0)
                    v0 = 2;
            }
            else if (w[2] <= 0 && w[0] <= 0)
            {
                v0 = 1;
                if (face_obt[1] == 1 && (xp - face[3]) * (face[0] - face[3]) + (yp - face[4]) * (face[1] - face[4]) > 0)
                    v0 = 0;
            }
            else if (w[0] <= 0 && w[1] <= 0)
            {
                v0 = 2;
                if (face_obt[2] == 1 && (xp - face[6]) * (face[3] - face[6]) + (yp - face[7]) * (face[4] - face[7]) > 0)
                    v0 = 1;
            }
            else if (w[0] <= 0)
                v0 = 1;
        ...
        int image_size)
    {
        ...
        scalar_t face_inv_determinant = (p[2][0] * (p[0][1] - p[1][1]) +
                                         p[0][0] * (p[1][1] - p[2][1]) +
                                         p[1][0] * (p[2][1] - p[0][1]));
        ...
        bool double_side)
    {
        ...
        for (int fn = 0; fn < nf; fn++)
        {
            ...
            if (check_border(xp, yp, face, sqrt(threshold)))
                continue; // triangle too far away from pixel
            ...
            if (func_id_alpha == 0)
            { // hard assign
                if (soft_fragment > 0.5)
                    soft_color[3] = 1.;
            }
            else if (func_id_alpha == 1)
            { // Sum
                soft_color[3] += soft_fragment;
            }
            else if (func_id_alpha == 2)
            { // Logical-Or
            ...
            if (zp < near || zp > far)
                continue; // triangle out of screen, pass
        ...
        bool double_side)
    {
        ...
        for (int fn = 0; fn < nf; fn++)
        {
            ...
            { // euclidean distance
                ...
                dis = dis_x * dis_x + dis_y * dis_y;
                if (sign < 0 && dis >= threshold)
                    continue; // ignore triangle too far away from the pixel
                soft_fragment = 1. / (1. + exp(-sign * dis / sigma_val));
            }
            ...
            // aggregate for rgb channels
            if (func_id_rgb == 0)
            { // Hard assign, no gradient to xyz
                if (fn == softmax_max)
                {
                    for (int k = 0; k < 3; k++)
                    {
            ...
            else if (func_id_rgb == 1 && (check_face_frontside(face) || double_side))
            { // Softmax (Z * D)
